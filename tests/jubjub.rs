...
use gennaro_dkg::{SecretParticipant, RefreshParticipant, Parameters, GroupHasher, AnyParticipant, RoundOutputGenerator, ParticipantRoundOutput, Round};

#[test]
fn dkg_and_refresh() {
    const REKEYS: usize = 25;
    ...
    for i in 0..REKEYS {
        let mut participants = Vec::<Box<dyn AnyParticipant<jubjub_plus::SubgroupPoint> + 'static>>::with_capacity(PARTIES);
        for j in 1..=PARTIES {
            let p = SecretParticipant::<jubjub_plus::SubgroupPoint>::new(IdentifierPrimeField(jubjub_plus::Scalar::from(j as u64)), &params).unwrap();
        ...
        let mut shares = participants.iter().map(|p| p.get_secret_share().unwrap()).collect::<Vec<gennaro_dkg::SecretShare<jubjub_plus::Scalar>>>();
        let secret = (&shares[..THRESHOLD]).combine().unwrap();
        assert_eq!(jubjub_plus::SubgroupPoint::generator() * secret.0, participants[0].get_public_key().unwrap(), "Secret key does not match the public key, rekey {}", i);

        for j in 0..REFRESHES {
            let mut refresh_parties = Vec::<Box<dyn AnyParticipant<jubjub_plus::SubgroupPoint> + 'static>>::with_capacity(PARTIES);
            for k in 1..=PARTIES {
                let p = RefreshParticipant::<jubjub_plus::SubgroupPoint>::new(IdentifierPrimeField(jubjub_plus::Scalar::from(k as u64)), &params).unwrap();
                refresh_parties.push(Box::new(p));
            }

            for _ in [Round::One, Round::Two, Round::Three, Round::Four] {
                let round_generators = run_next_round::<jubjub_plus::SubgroupPoint>(&mut refresh_parties);
                receive_round_output(&mut refresh_parties, &round_generators);
            }
            let refresh_shares = refresh_parties.iter().map(|p| p.get_secret_share().unwrap()).collect::<Vec<gennaro_dkg::SecretShare<jubjub_plus::Scalar>>>();
            let refresh_secret = (&refresh_shares[..]).combine().unwrap();
            assert_eq!(refresh_secret.0, jubjub_plus::Scalar::zero(), "Refresh secret is not zero, rekey {}, refresh {}", i, j);

            let mut cloned_shares = shares.clone();
            for (share, refresh_share) in cloned_shares.iter_mut().zip(refresh_shares.iter()) {
                assert_eq!(share.identifier.0, refresh_share.identifier.0, "Share identifiers do not match, rekey {}, refresh {}", i, j);
                share.value.0 += refresh_share.value.0;
            }

            let refreshed_secret = (&cloned_shares[..THRESHOLD]).combine().unwrap();
            assert_eq!(refreshed_secret.0, secret.0, "Refreshed secret does not match the original secret, rekey {}, refresh {}", i, j);
